...
class Client(slixmpp.ClientXMPP):
    ...
    async def session_start(self, event):
        ...
        self.neighbors = get_neighbors(
            topology_file=self.topo_file, ID=self.nickName)
        self.neighborsTimeSend = np.zeros(len(self.neighbors))

        self.table = pd.DataFrame(
            index=['neighbour', 'distance'], columns=self.neighbors + [self.nickName])
        ...
        if note['type'] == 'chat':
            ...
            if message["recipientNode"] == self.jid:
                ...
                self.send_message(
                    mto=get_JID(
                        ...
                        ID=self.table[get_ID(
                            names_file=self.names_file, JID=message["recipientNode"])]['neighbour']
                    ),
            ...
            if message['type'] == 'ecoSend':
                ...
                if(time.time() - message["sendTime"] > 1):
                    ...
                    self.table.at['distance', message["recipientNode"]] = (
                        time.time() - message["sendTime"]) / 2
                ...
                for node in table.keys():
                    ...
                    if node != self.nickName:
                        ...
                        d = min(
                            (self.table.at['distance', senderNode] + table[node]), self.table.at['distance', node])
